...
*/
struct Args {
    std::string input, tree, output; // struktura sklada sie z 3 stringow
};

/**
 * @brief Structure of a node extracted from the tree file.
 */
 //struktura jednego wezla
struct Node {
    std::string key;
    std::string argument;
    std::string op;
...
//tworzy wektor ktory ma w sobie mape ktore skaldaja sie ze strignow i doubli w ktorym sa dane personalne czyli wzrost wyskok
typedef std::vector<std::map<std::string, double> > PersonData;
// definuje to jako pearsondata
/**
